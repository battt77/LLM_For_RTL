...
module TopModule (
    ...
    input signed [23:0] x0_real, x0_imag,
    input signed [23:0] x1_real, x1_imag,
    input signed [23:0] x2_real, x2_imag,
    input signed [23:0] x3_real, x3_imag,
    input signed [23:0] x4_real, x4_imag,
    input signed [23:0] x5_real, x5_imag,
    input signed [23:0] x6_real, x6_imag,
    input signed [23:0] x7_real, x7_imag,

    output yout_valid,
    output signed [23:0] y0_real, y0_imag,
    output signed [23:0] y1_real, y1_imag,
    output signed [23:0] y2_real, y2_imag,
    output signed [23:0] y3_real, y3_imag,
    output signed [23:0] y4_real, y4_imag,
    output signed [23:0] y5_real, y5_imag,
    output signed [23:0] y6_real, y6_imag,
    output signed [23:0] y7_real, y7_imag
);
    ...
    for (m = 0; m <= 2; m = m + 1) begin: stage
        for (k = 0; k <= 3; k = k + 1) begin: unit
            SubModule u_butter (
                ...
                .xq_real     (xm_real[m][(k[m:0] < (1<<m) ? (k[3:m] << (m+1)) + k[m:0] : (k[3:m] << (m+1)) + (k[m:0]-(1<<m))) + (1<<m)]),
